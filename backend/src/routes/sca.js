...
// POST /api/sca/scan — run SCA scan before merge
// Fetches changed files from GitHub, analyzes for vulnerabilities
router.post('/scan', async (req, res) => {
  try {
    const { owner, repo, base, head } = req.body;
    if (!owner || !repo || !base || !head) {
      return res.status(400).json({ error: 'owner, repo, base, head required' });
    }
        ...
        if (depFile.endsWith('package.json')) {
          try {
            ...
            for (const [name, version] of Object.entries(allDeps || {})) {
              // Flag known vulnerable patterns
    ...
    for (const srcFile of sourceFiles.slice(0, 20)) { // Limit to 20 files
      try {
        const contentRes = await fetch(
    ...
    // Block merge if critical or high findings
...
// GET /api/sca/config — get SCA configuration
